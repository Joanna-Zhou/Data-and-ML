...
def kNNPerformance(datasetName, modificationRange, distanceHeuristic='l2', k=5):
    '''
    ...
    '''
    kNNtest = kNNTraining(datasetName, distanceHeuristic, k, 1)
    runtimes = []
    ...
    plt.bar(xAxis, height= runtimes, alpha=0.5, color=_COLORS)
    plt.xticks(xAxis, ['a', 'b', 'c', 'd'][:len(modificationRange)])
    ...
    for x in xAxis:
        plt.text(x=x-0.1, y=runtimes[x]+0.001, s='%1.2f'%(runtimes[x]), size = 9)
...
if __name__ == '__main__':
    runtimes = kNNPerformance('mauna_loa', range(1,4)) #rosenbrock
